...
patch -f -p1 < ../sourcealyzer.patch

echo -e "${GREEN}Sanity checking the patch${NC}"
grep "sandbox" clang/include/clang/Driver/Options.td
cd build

echo -e "${GREEN}Running cmake${NC}"
cmake ../llvm -G Ninja -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS="clang;lld" -DENABLE_EXPERIMENTAL_NEW_PASS_MANAGER=true
check_exit_status

echo -e "${GREEN}Executing ninja${NC}"
ninja -j 10 # lower `j` required because otherwise the many jobs might cause OOM
check_exit_status
